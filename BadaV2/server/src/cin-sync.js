      ...
      if (!err) {
        ...
        Database.mysql.query(sql, connection, function (err, results) {
          if (!err) {
            ...
            insertData = null;
            let logMessage = "Latest CIN INSERT success";
  ...
  redisClient.hget("storage", JSON.stringify(parentResource), (err, val) => {
    ...
    if (val) {
      if (val == 0) {
        ...
        ProdKafka("spatialdata", mobiusReturn);
...
async function ProdKafka(kafkaTopic, bodyString) {
  ...
  await producer.send({
    topic: kafkaTopic,
    messages: [{ key: bodyString["ri"].substring(1), value: JSON.stringify(bodyString) }],
  });
}
