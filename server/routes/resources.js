...
let saveResource = (type, data, userId, path, storage, connection, datamodel = null) => {
  let insertData = {};
  ...
  if (type === "cnt") {
    ...
    if (storage.timeseries === "true") redisCacheValue += 1;
    if (storage.spatialdata === "true") redisCacheValue += 2;
...
let deleteResource = (type, resourceId, connection) => {
  ...
  Database.mysql.query(sql, connection, (err, result) => {
    ...
    if (result.affectedRows === 1) {
      console.log(`Success the ${type} ${resourceId} Deletion`);
    }
  });
};
let deleteInfluxData = (ae, cnt = null) => {
  let sql = `drop measurement ${ae}`;
  let logMessage = `delete Influxdb data [${ae}]`;
  if (cnt != null) {
    sql = `delete from ${ae} where cnt = '${cnt}'`;
    logMessage = `delete Influxdb data [${ae}/${cnt}]`;
  }
  Database.timeseries.query(sql);
...
router.post("/cnt", (req, res) => {
  ...
  console.log("data model test - req.body : ", req.body);
  ...
  let datamodel = req.body.datamodel;
      ...
      response
        ...
        .on("end", async () => {
          ...
          if (response.statusCode === 201) {
            Database.mysql.connection((code, connection) => {
              if (code === "200") {
                saveResource(type, resultData, userId, pathString, storage, connection, datamodel);
                saveCount(type, resultData, userId, connection);
                connection.release();
                res.status(response.statusCode).json(resultData);
                responseTime.store(res.getHeader("X-Response-Time"), POST);
...
router.delete("/cnt", (req, res) => {
  ...
  redisClient.hdel("latest", `/Mobius/${parentResource.ae}/${parentResource.cnt}`, function (lerr, lsuccess) {
    if (lerr) {
      console.error(`Failed to remove latest in redis [${parentResource.ae}/${parentResource.cnt}] : ${lerr}`);
    }

    redisClient.hdel("storage", JSON.stringify({ ae: parentResource.ae, cnt: parentResource.cnt }), function (serr, ssuccess) {
      if (serr) {
        console.error(`Failed to remove storage in redis [${parentResource.ae}/${parentResource.cnt}] :` + serr);
      }

      redisClient.hdel("datamodel", `/${parentResource.ae}/${parentResource.cnt}`, function (derr, dsuccess) {
        if (derr) {
          console.error(`Failed to remove datamodel in redis [${parentResource.ae}/${parentResource.cnt}] : ` + derr);
        }
        console.log(`delete Redis Latest : ${lsuccess}, storage : ${ssuccess}, datamodel : ${dsuccess} [${parentResource.ae}/${parentResource.cnt}] `);
      });
    });
  });

  deleteInfluxData(parentResource.ae, parentResource.cnt);
          ...
          if (response.statusCode === 200) {
            Database.mysql.connection((code, connection) => {
              if (code === "200") {
                ...
                deleteInfluxData(parentResource.ae);
                  ...
                  if (result) {
                    result.forEach((element) => {
                      ...
                      Database.mysql.query(sql, connection, (err, result) => {
                        if (err) console.log(err);
                        if (result.affectedRows >= 1) {
                          console.log(`Success ${parentResource.ae}/${parentResource.cnt} cnt deletion`);
                          // delete latestcin
