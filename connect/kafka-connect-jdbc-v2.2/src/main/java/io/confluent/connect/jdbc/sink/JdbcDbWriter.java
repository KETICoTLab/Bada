  ...
  void write(final Collection<SinkRecord> records)
    ...
    try {
      ...
      for (SinkRecord record : records) {
        ...
        /*
        System.out.println();
        System.out.println("JDBC DB WRITER");
        System.out.println("THIS IS PGSQL TOPIC : " + record.topic());
        System.out.println("THIS IS PGSQL TOPIC : " + String.format("%s_%s", ae, cnt));
        System.out.println();

         */

        //final TableId tableId = destinationTable(record.topic());
        final TableId tableId = destinationTable(String.format("%s_%s", ae, cnt));
        BufferedRecords buffer = bufferByTable.get(tableId);
        if (buffer == null) {
          buffer = new BufferedRecords(config, tableId, dbDialect, dbStructure, connection);
          bufferByTable.put(tableId, buffer);
        }
        System.out.println("HERE IS BUFFER ADD");
        buffer.add(record);
      }
