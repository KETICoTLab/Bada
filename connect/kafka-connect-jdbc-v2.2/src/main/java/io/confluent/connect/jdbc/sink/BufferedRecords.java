...
public class BufferedRecords {
  ...
  public List<SinkRecord> add(SinkRecord record) throws SQLException, TableAlterOrCreateException {
    ...
    if (schemaChanged || updateStatementBinder == null) {
      ...
      // Mobius con field
      Map<String, Object> conField = (Map<String, Object>) record.value();

      SchemaBuilder builder = SchemaBuilder.struct();
      builder.field("applicationentity", Schema.STRING_SCHEMA);
      builder.field("container", Schema.STRING_SCHEMA);
      for (String key : conField.keySet()) {
        if (key.equals("longitude") || key.equals("latitude") || key.equals("altitude")) {
          builder = builder.field(key, Schema.FLOAT64_SCHEMA);
        } else {
          builder = builder.field(key, Schema.STRING_SCHEMA);
        }
      }
    ...
    /*
    System.out.println();
    System.out.println("BUFFERED RECORDS ADD FUNC");
  ...
  public List<SinkRecord> flush() throws SQLException {
    ...
    for (SinkRecord record : records) {
      /*
      System.out.println();
      System.out.println("Buffered Records");
      System.out.println(record);
       */
      ...
      for (String key : conField.keySet()) {
        if (key.equals("longitude") || key.equals("latitude") || key.equals("altitude")) {
          valueStruct.put(key, checkDoubleType(conField.get(key)));
        } else {
          valueStruct.put(key, conField.get(key).toString());
        }
      }
      ...
      String ae = pi[2];
      String cnt = pi[3];
      valueStruct.put("applicationentity", ae);
      valueStruct.put("container", cnt);
      ...
      Struct valueStruct = new Struct(valueSchema)
              .put("applicationentity", ae)
              .put("container", cnt)
              .put("latitude", checkDoubleType(conField.get("latitude")))
              .put("longitude", checkDoubleType(conField.get("longitude")))
              .put("altitude", checkDoubleType(conField.get("altitude")))
              .put("data", record.value().toString());
