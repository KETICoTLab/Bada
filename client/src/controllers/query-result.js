...
// let initStartDate = new Date(new Date() - (6*60*60*1000)); // 6 hourse ago
...
export default {
  ...
  mounted() {
    ...
    for (let i = 8; i > 0; i--) {
      let date = new Date(currentDate - timeInterval * i * 1000);
    ...
    setInterval(() => {
      ...
      this.chartOption.series[0].data.push(Math.floor(Math.random() * 5));
    }, timeInterval * 1000);
  }
};
